...
@test "-c must come after lockfile" {
	...
	# This should fail or be treated as a different mode
	[ "$status" -ne 0 ] || true
}
...
@test "-o closes fd before exec so grandchildren don't inherit lock" {
	# With -o, the lock fd is closed in the child before exec.
	# The parent flock still holds the lock, so the lock remains held.
	# But the child process itself doesn't have the fd open, which means
	# grandchild processes won't inherit it.
	# Verify the child runs fine without the fd.
	result=$(${FLOCK} -o ${LOCKFILE} echo run)
	[ "$result" = run ]
}
...
@test "--verbose produces output about lock acquisition" {
